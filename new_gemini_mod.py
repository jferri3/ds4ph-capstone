...
if analysis_option == "Abundance Analysis":
    if merged_data is not None:  # Use the same merged_data from the "Set-up Check" section
        ...
        if len(selected_simple_ids) > 0:
            ...
            top_10_species_data.index.name = 'SimpleID'  # Make sure the index is named
            grouped_df = top_10_species_data.stack().reset_index(name='Counts')
            grouped_df.columns = ['SimpleID', 'Species', 'Counts']  # Correct column names
            st.dataframe(grouped_df)
        else:
